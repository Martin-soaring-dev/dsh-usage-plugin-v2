...
`dsh-usage-plugin-v2` — DeepSeek Harness 用量与消耗插件的独立复刻增强版
...
- **仅通过 GitHub 发行**：本 fork 不再尝试发布 npm；GitHub Releases 成为唯一安装与更新源。
- **手动检查更新**：设置页新增“插件更新”，严格比较当前版本与仓库最新正式 Release。
- **固定版本安装**：用户确认后只安装刚刚检查到的 Release tag，完成后要求完全重启 Harness 服务；失败不会自动重试。
- **更新安全校验**：拒绝草稿、预发布、非法版本号、来源不匹配和检查后发生变化的 tag。
- **清理发布流程**：删除已失效的 npm 发布工作流和请求/结果文件。

## v1.11.0 (2026-08-18)

- **独立 fork 发行**：npm 包改用本项目独立名称 `dsh-usage-plugin-v2`，仓库元数据、插件 ID 与安装命令均指向 `Martin-soaring-dev/dsh-usage-plugin-v2`；不再占用或冒用上游 `@feiyang666` scope。
