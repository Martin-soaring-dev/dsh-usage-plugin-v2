  ...
  factory: (require) => {
    ...
    function BalancePanel() {
      ...
      var providers = [
        ...
        { id: "siliconflow", name: "SiliconFlow", hint: "模型提供商的 apiKeyEnv，或 SILICONFLOW_API_KEY（推理 Key）", actionUrl: "https://cloud.siliconflow.cn/account/ak", actionLabel: "打开 SiliconFlow API 密钥" },
        { id: "digitalocean", name: "DigitalOcean", hint: "DIGITALOCEAN_TOKEN（账户级 PAT，需 billing:read；不是 DO AI 推理 Key）", actionUrl: "https://cloud.digitalocean.com/account/api/tokens", actionLabel: "创建 billing:read Token" },
      ...
      return el("div", { style: st.root },
        el("div", { style: st.head },
          ...
          el("div", { style: st.actions },
            selected.unsupported
              ...
              : el("button", { style: s.status === "loading" ? st.btnDisabled : st.btn, disabled: s.status === "loading", onClick: query }, s.status === "loading" ? "查询中…" : "查询余额")
          )
        ),
        el("div", { style: st.subtabBar },
        ...
        d && d.ok
          ? el("div", { style: st.hero },
              el("div", { style: st.heroLabel }, d.providerName + " · 账户余额"),
              el("div", { style: st.heroValue }, symbol + " " + fmtBalance(d.totalBalance)),
              el("div", { style: st.heroCurrency }, d.currency + (d.isAvailable === true ? " · 账户可用" : d.isAvailable === false ? " · 账户不可用" : " · 查询成功")),
        ...
        d && d.ok && d.provider === "siliconflow" && d.zeroBalance
          ? el("div", { style: st.infobox },
              el("div", { style: st.infoboxTitle }, "接口已成功返回 ¥0.00"),
              el("div", null, "这不是请求失败。若 SiliconFlow 控制台显示非零余额，请核对下方的模型提供商、凭据来源与官方 API 端点是否对应同一账户。"),
