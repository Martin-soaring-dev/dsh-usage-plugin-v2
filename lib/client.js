window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function UpdatePanel() {
      ...
      function check() {
        setS({ status: "checking", data: null, error: "" });
        api({ action: "updateCheck" }).then(function (res) {
          if (res && res.ok) setS({ status: "checked", data: res, error: "" });
          else setS({ status: "error", data: null, error: (res && res.error) || "检查更新失败" });
        }).catch(function (e) {
          setS({ status: "error", data: null, error: String((e && e.message) || e) });
        });
      }
      ...
      function install() {
        if (!s.data || !s.data.tag) return;
        setS({ status: "installing", data: s.data, error: "" });
        api({ action: "updateInstall", tag: s.data.tag }).then(function (res) {
          if (res && res.ok) setS({ status: "installed", data: Object.assign({}, s.data, res), error: "" });
          else setS({ status: "error", data: s.data, error: (res && res.error) || "安装更新失败" });
        }).catch(function (e) {
          setS({ status: "error", data: s.data, error: String((e && e.message) || e) });
        });
      }
      ...
      return el("div", { style: st.root },
        el("div", { style: st.head },
          el("div", null,
            el("div", { style: st.title }, "插件更新"),
            el("div", { style: st.sub }, "仅从 Martin-soaring-dev/dsh-usage-plugin-v2 的正式 GitHub Release 更新")
          ),
          el("button", { style: s.status === "checking" || s.status === "installing" ? st.btnDisabled : st.btn, disabled: s.status === "checking" || s.status === "installing", onClick: check }, s.status === "checking" ? "检查中…" : "检查更新")
        ),
        d ? el("div", { style: st.cards },
          el(Card, { label: "当前版本", value: d.currentVersion || "—" }),
          el(Card, { label: "GitHub 最新版本", value: d.latestVersion || "—", hint: d.publishedAt || "" })
        ) : null,
        s.status === "checked" && d && !d.updateAvailable
          ? el("div", { style: st.infobox, marginTop: 10 }, el("div", { style: st.infoboxTitle }, "当前已是最新版本"))
          : null,
        d && d.updateAvailable && s.status !== "installed"
          ? el("div", { style: st.infobox, marginTop: 10 },
              el("div", { style: st.infoboxTitle }, "发现新版本 " + d.latestVersion),
              el("div", null, "点击安装后会从该 Release tag 下载并替换当前插件文件。安装完成后必须完全重启 Harness 服务。"),
              el("div", { style: st.actions, marginTop: 8 },
                el("button", { style: s.status === "installing" ? st.btnDisabled : st.btnPrimary, disabled: s.status === "installing", onClick: install }, s.status === "installing" ? "安装中…" : "安装此版本"),
                el("a", { style: st.actionLink, href: d.releaseUrl, target: "_blank", rel: "noreferrer" }, "查看 Release")
              )
            )
          : null,
        s.status === "installed"
          ? el("div", { style: st.infobox, marginTop: 10 },
              el("div", { style: st.infoboxTitle }, "更新文件已安装"),
              el("div", null, "请停止 Harness 服务并重新启动；只刷新网页不会加载新版本。")
            )
          : null,
        s.error ? el("div", { style: st.errbox, marginTop: 10 }, el("div", { style: st.errboxTitle }, "更新失败"), el("div", null, s.error), el("div", { style: st.note, marginTop: 6 }, "没有自动重试。请保留此信息后共同排查。")) : null
      );
    }
    ...
    function apply(ctx) {
      ...
      slots.inject("settings.section", function () {
        return slots.register(
          { name: "settings.section", id: "plugin-update", order: 32, label: "插件更新" },
          function () { return el(UpdatePanel, null); }
        );
      });
    }
