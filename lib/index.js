...
export default {
  ...
  apply(ctx) {
    ...
    try {
      ...
      const dirs = () => ({
        data: root + '\\dsh-usage',
        csv: root + '\\dsh-usage\\csv',
        json: root + '\\dsh-usage\\json',
        images: root + '\\dsh-usage\\images'
      })
      ...
      async function tryInitWithRoot(candidate, policy) {
        const tryPath = normPath(candidate) + '\\dsh-usage\\usage-records.json'
        try {
        ...
        try {
          ...
          dataPath = tryPath
          pricingPath = tryPath.slice(0, tryPath.length - 'usage-records.json'.length) + 'pricing.json'
      ...
      async function migrateLegacy(candidates) {
        ...
        for (const c of candidates) {
          paths.push(normPath(c) + '\\.dsh-usage-records.json')
          paths.push(normPath(c) + '\\dsh-usage\\usage-records.json')
        }
      ...
      async function spawnNode(script, stdinData, env) {
        ...
        let exe = null
        for (const c of ['node.exe', 'node', 'C:\\Program Files\\nodejs\\node.exe']) {
        ...
        try {
          handle = subprocess.spawn({
            argv: [exe, '-e', script],
            cwd: root || 'C:\\',
            stdio: stdinData != null
              ? { stdin: { data: stdinData }, stdout: { maxBytes: 65536 }, stderr: { maxBytes: 65536 } }
        ...
        if (outcome.exitCode !== 0) return { ok: false, error: 'node 退出码 ' + outcome.exitCode + (errText ? '：' + errText.trim() : '') }
        return { ok: true, out: outText }
      }
      ...
      async function pickDirectory() {
        ...
        try {
          handle = subprocess.spawn({
            argv: [exe, '-NoProfile', '-STA', '-NonInteractive', '-Command', script],
            cwd: root || 'C:\\',
            stdio: { stdin: 'ignore', stdout: { maxBytes: 65536 }, stderr: { maxBytes: 65536 } },
            graceMs: 120000
          })
        } catch (e) { return fail('启动失败：' + msg(e)) }
        let outcome
        try { outcome = await handle.done } catch (e) { return fail('执行失败：' + msg(e)) }
        const outText = handle.collected && handle.collected.stdout ? handle.collected.stdout.readFrom(0).text : ''
        const path = normPath(outText.trim())
      ...
      async function revealDir(dirArg) {
        ...
        if (isKey) {
          ...
          const policy = sessionPolicy()
          try {
            const t = await fs.resolve(target + '\\.keep')
        ...
        let exe = null
        try { exe = await subprocess.resolveExecutable('explorer.exe') } catch (e) {}
        if (!exe) return fail('未找到 explorer.exe')
        try {
          subprocess.spawn({ argv: [exe, target], cwd: root || 'C:\\', stdio: { stdin: 'ignore', stdout: { maxBytes: 1024 }, stderr: { maxBytes: 1024 } }, graceMs: 5000 })
          return { ok: true }
      ...
      async function routeApi(body) {
        ...
        switch (action) {
          ...
          case 'exportPng': {
            ...
            const dirArg = body && body.dir ? normPath(String(body.dir)) : ''
            const outPath = normPath((dirArg || dirs().images) + '\\' + name)
