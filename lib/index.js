  ...
  apply(ctx) {
    ...
    try {
      ...
      async function configuredModelProvider(provider) {
        ...
        try {
          // `settings` is intentionally optional so older Harness versions can
          // keep using the plugin's conventional environment-variable names.
      ...
      async function queryBalance(providerId) {
        ...
        if (!hit) {
          const message = provider.id === 'digitalocean'
            ? '未配置 DIGITALOCEAN_TOKEN（账户级 Personal Access Token，需 billing:read 权限；不是 DO AI 推理 Key），请创建并配置后重试'
