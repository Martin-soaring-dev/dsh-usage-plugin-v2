...
const PROFILE_ROOT = profileRootFromModule(import.meta.url)
export default {
  ...
  apply(ctx) {
    ...
    try {
      ...
      // Provider-specific verified prices (CNY / 1M tokens), checked 2026-08-19.
      ...
      // DigitalOcean Serverless Inference prices (USD / 1M tokens), verified 2026-08-19.
      ...
      async function runCollect(argv, opts) {
        ...
        try {
          handle = subprocess.spawn({
            argv,
            cwd: (opts && opts.cwd) || await safeCwd(),
      ...
      async function checkForUpdate() {
        const script = [
          'const https=require("https");',
          'const url=process.env.DSH_UPDATE_URL;',
          'const req=https.get(url,{headers:{Accept:"application/vnd.github+json","User-Agent":"dsh-usage-plugin-v2","X-GitHub-Api-Version":"2022-11-28"}},function(res){',
          'let body="";',
        ...
        const result = await spawnNode(script, null, { DSH_UPDATE_URL: RELEASE_API_URL })
        if (!result.ok) return fail('检查 GitHub Release 失败：' + result.error)
        let response
        try { response = JSON.parse(result.out) } catch (e) { return fail('无法解析 GitHub 检查结果') }
        if (response.error) return fail('无法连接 GitHub：' + response.error)
        if (response.statusCode !== 200) return fail('GitHub Releases API 返回 HTTP ' + response.statusCode)
        try {
          return { ok: true, ...releaseInfo(JSON.parse(response.body), PACKAGE_VERSION) }
        } catch (e) {
          return fail(msg(e))
        }
      }

      async function installUpdate(requestedTag) {
      ...
      async function configuredModelProvider(provider) {
        ...
        try {
          // SiliconFlow intentionally requires this model-provider lookup;
          // other direct providers may still use their conventional key name.
      ...
      async function balanceCredentialStatus(providerId) {
        ...
        if (provider.id === 'siliconflow') {
          ...
          if (!profile) {
            return balanceFailure(provider,
              '未在“设置 → 模型”中找到 Provider ID 或显示名为 siliconflow 的模型提供商。', {
                errorCode: 'model-provider-missing'
              })
          }
          if (!profile.apiKeyEnv) {
            return balanceFailure(provider,
              '模型提供商 ' + profile.route + ' 没有配置 apiKeyEnv；请编辑该模型提供商并保存 API Key。', {
                errorCode: 'model-credential-ref-missing',
      ...
      async function saveBalanceCredential(providerId, rawValue) {
        const provider = getBalanceProvider(providerId)
        if (!provider || provider.id !== 'digitalocean') {
          return fail('仅支持在余额页保存 DigitalOcean 账户 Token')
        }
        const value = String(rawValue || '').trim()
        if (!/^dop_v1_[A-Za-z0-9_-]{20,}$/.test(value)) {
          return balanceFailure(provider,
            'Token 格式不正确：请输入 DigitalOcean 控制台创建的 dop_v1_ Personal Access Token，不要使用 DO AI 推理 Key。', {
              errorCode: 'invalid-credential-format'
            })
        }
        const credentials = ctx.get('credentials')
        ...
        if (info.configured && !info.writable) {
          return balanceFailure(provider,
            'DIGITALOCEAN_TOKEN 当前由只读来源 ' + (info.source || '环境变量') + ' 提供，不能在页面覆盖；请修改该来源后重启。', {
              errorCode: 'credential-read-only',
        ...
        if (!saved.configured) {
          return balanceFailure(provider, 'Token 保存后未能从凭据服务中重新读取', { errorCode: 'credential-save-unverified' })
        }
        return {
          ok: true,
          provider: provider.id,
      ...
      async function queryBalance(providerId) {
        ...
        if (provider.queryMode === 'unsupported') {
          return balanceFailure(provider,
            'AMD GPU Cloud 当前未公开可由推理 API Key 调用的余额查询端点；请在 AMD Developer Cloud 控制台查看 credits。', {
            unsupported: true,
        ...
        if (provider.id === 'siliconflow' && !modelProfile) {
          return balanceFailure(provider,
            '未在“设置 → 模型”中找到 Provider ID 或显示名为 siliconflow 的模型提供商。请先添加该提供商、填写 API Key 并保存，然后返回此页查询。', {
              errorCode: 'model-provider-missing'
            })
        }
        if (provider.id === 'siliconflow' && modelProfile && !modelProfile.apiKeyEnv) {
          return balanceFailure(provider,
            '模型提供商 ' + modelProfile.route + ' 没有配置 API Key。请在“设置 → 模型”中编辑该提供商并保存 API Key。', {
              errorCode: 'model-credential-ref-missing',
              modelProviderRoute: modelProfile.route
            })
        }
        if (modelProfile && modelProfile.apiKeyEnv) {
          credentialCandidates.push({ name: modelProfile.apiKeyEnv, route: modelProfile.route })
        }
        if (provider.id !== 'siliconflow') {
        ...
        if (!hit) {
          const message = provider.id === 'digitalocean'
            ...
            : '未找到 ' + provider.credentialHint + (modelProfile && modelProfile.apiKeyEnv
                ? '；也未能解析模型提供商 ' + modelProfile.route + ' 的 ' + modelProfile.apiKeyEnv
                : '') + '，请配置后重试'
          return balanceFailure(provider, message, {
            errorCode: 'missing-credential',
            modelProviderRoute: modelProfile ? modelProfile.route : ''
          })
        }
        const key = hit.value
        ...
        const r = await spawnNode(script, null, { BALANCE_API_KEY: key, BALANCE_API_URL: endpoint })
        ...
        if (parsed.statusCode !== 200) {
          const authHint = parsed.statusCode === 401 || parsed.statusCode === 403
            ? ' 请检查凭据是否属于该账户、是否有效及是否具备余额/账单读取权限。'
            : ''
          return balanceFailure(provider,
            '接口返回 HTTP ' + parsed.statusCode + '：' + String(parsed.body || '').slice(0, 300) + authHint, {
        ...
        if (!String(parsed.contentType || '').toLowerCase().includes('application/json')) {
          return balanceFailure(provider,
            '接口返回了非 JSON 内容（Content-Type: ' + String(parsed.contentType || '未知') + '），请求可能被网络代理拦截。', {
      ...
      async function routeApi(body) {
        ...
        // API requests carry the active Harness session even before the first
        // model call. Re-resolve its workspace here so a freshly restarted
        // plugin loads the complete persisted history before serving the panel.
        try { await ensureSessionRoot() } catch (e) {}
        switch (action) {
          case 'list': {
            try { await refreshFxRate(false) } catch (e) {}
            ...
            return { ok: true, records: items, count: items.length, dataPath, persistOk, persistError, pricing: PRICING, effectiveAt: EFFECTIVE_AT, fx: FX, days: buildDays() }
          }
          case 'fxRefresh': {
          ...
          case 'updateInstall':
            return installUpdate(body && body.tag)
          case 'pickDir':
