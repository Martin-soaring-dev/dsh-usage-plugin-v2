...
import { getBalanceProvider, parseBalanceResponse, providerList } from './balance.js'
export default {
  ...
  apply(ctx) {
    ...
    try {
      ...
      async function resolveCredential(credentials, names) {
        for (const name of names) {
          try {
            const hit = await credentials.resolve(name)
            if (hit && hit.value) return { name, value: hit.value }
          } catch (e) {}
        }
        return null
      }
      ...
      async function queryBalance(providerId) {
        ...
        if (provider.queryMode === 'unsupported') {
          return {
            ...
            error: 'AMD GPU Cloud 当前未公开可由 API Key 调用的余额查询端点；请在服务商控制台查看余额。'
          }
        }
        const credentials = ctx.get('credentials')
        if (!credentials) return fail('凭据服务不可用')
        const hit = await resolveCredential(credentials, provider.credentialNames)
        if (!hit) return fail('未配置 ' + provider.credentialHint + '，请配置后重试')
        const key = hit.value
        const script = [
        ...
        const r = await spawnNode(script, null, { BALANCE_API_KEY: key, BALANCE_API_URL: provider.endpoint })
        ...
        if (normalized.ok) normalized.credentialName = hit.name
        return normalized
      }
